...
function PostgreSQL(postgresql, settings) {
  ...
  this.pg.on('error', function(err) {
    debug('Idle client error: %s', err.message);
  });
    ...
    connection.query(sql, params, function(err, data) {
      ...
      if (done) {
        process.nextTick(function() {
          ...
          // Never pass err here — done(err) destroys the connection
          // instead of returning it, causing pool churn under load.
          done();
        });
      }
      var result = null;
...
function unaccent(sql, queryType) {
  // Check REGEX (~*) & SELECT from query and add unaccent custom query
  if (sql.indexOf("~*") !== -1 && sql.indexOf(queryType) !== -1) {
    // Split from " "
    var _sql = sql.split(" ");
    var regexPos = _sql.indexOf("~*");

    // Split left side from ""
    var _sqlLeft = [];
    var leftType = '\"';
    _sqlLeft = _sql[regexPos-1].split(leftType);

    // Get attr index position
    var _attrPos = -1;
    _sqlLeft.forEach(function (_sp, spIndex) {
        if (new RegExp("^[a-zA-Z0-9\_\-]+$").test(_sp)) _attrPos = spIndex;
    });

    // Convert back leftSide
    _sql[regexPos-1] = '';
    _sqlLeft.forEach(function (_sp, spIndex) {
        if (spIndex === _attrPos) _sql[regexPos-1] = _sql[regexPos-1] + 'unaccent(\"' + _sp + '\")';
        else if (spIndex < _attrPos) _sql[regexPos-1] = _sp + _sql[regexPos-1];
        else _sql[regexPos-1] = _sql[regexPos-1] + _sp;
    });

    // Split right side from ) and convert back rightSide
    var _sqlRight = [];
    var rightType = ")";
    var _parenthesisSplit = _sql[regexPos+1].split(rightType);
    if (_parenthesisSplit.length >= 2) {
      _parenthesisSplit.forEach(function (_sp, spIndex) {
           if (spIndex === 0) _sql[regexPos+1] = 'unaccent(' + _sp + ')';
           else _sql[regexPos+1] = _sql[regexPos+1] + ')';
      });
    }

    var _convSql = '';
    _sql.forEach(function (_str) {
        _convSql = _convSql + ' ' + _str;
    });
    // Merge sql splits
    return _convSql;
  }
...
PostgreSQL.prototype._buildWhere = function(model, where) {
  ...
  if (Array.isArray(where.customWHERE)){
		if (whereStmt.sql) {
			whereStmt.sql +=" "+where.customWHERE.join(' ');
		}else{
			whereStmt.sql =" TRUE "+where.customWHERE.join(' ');
		}
	}
...
PostgreSQL.prototype.buildSelect = function(model, filter, options) {
  ...
  let selectStmt = new ParameterizedSQL('SELECT ' +
    ...
    ' FROM ' + this.tableEscaped(model) + filter.join);
